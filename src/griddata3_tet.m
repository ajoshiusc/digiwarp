function w = griddata3_tet(x,y,z,tet,v,xi,yi,zi,tnum,pc,method,options)
...
if nargin < 10
  error('MATLAB:griddata3:NotEnoughInputs', 'Needs at least 10 inputs.');
end
...
switch lower(method(1)),
  case 'l'
    w = linear_tet(X,tet,v,[xi(:) yi(:) zi(:)],tnum,pc,opt);
  case 'n'
    w = nearest_tet(X,tet,v,[xi(:) yi(:) zi(:)],opt);
  otherwise
...
%------------------------------------------------------------
function zi = linear_tet(x,tri,y,xi,t,p,opt)
...
%------------------------------------------------------------
function zi = nearest_tet(x,tri,y,xi,opt)
